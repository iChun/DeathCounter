...
public class CommandDeathCounter
{
        ...
        LiteralCommandNode<CommandSourceStack> command =
            dispatcher.register(Commands.literal("dc")
                ...
                .then(Commands.literal("get")
                    .then(Commands.argument("name", StringArgumentType.word())
                        ...
                        .executes((source) -> {
                            ...
                            if(deaths > 0)
                            {
                                source.getSource().sendSuccess(new TranslatableComponent("commands.deathcounter.get", name, deaths, rank), false);
                            }
                            else
                            {
                                source.getSource().sendSuccess(new TranslatableComponent("commands.deathcounter.get.none", name), false);
                            }
                            return 0;
                        })))
                ...
                .then(Commands.literal("broadcast").requires((p) -> p.hasPermission(DeathCounter.config.commandPermissionLevel))
                    .executes((source) -> {
                        //send to all
                        source.getSource().sendSuccess(new TranslatableComponent("commands.deathcounter.leaderboard.broadcasted"), true);
                    ...
                    .then(Commands.argument("from", StringArgumentType.word())
                        ...
                        .then(Commands.argument("to", StringArgumentType.word())
                            ...
                            .executes((source) -> {
                                ...
                                if(deaths > 0)
                                {
                                    source.getSource().sendSuccess(new TranslatableComponent("commands.deathcounter.transfer", deaths, from, to), true);
                                }
                                else
                                {
    ...
    public static void broadcastLeaderboard(Collection<? extends Entity> entities, CommandSourceStack source, final int count)
    {
        ...
        {
            source.sendSuccess(new TranslatableComponent("commands.deathcounter.leaderboard"), false);
            if(DeathCounter.deathHandler.getRankings().isEmpty())
            {
                source.sendSuccess(new TranslatableComponent("commands.deathcounter.leaderboard.none"), false);
            }
            else
            {
                int done = 0;
                ...
                for(Map.Entry<Integer, TreeSet<String>> e : DeathCounter.deathHandler.getRankings().entrySet())
                {
                    ...
                    {
                        int finalRank = rank;
                        source.sendSuccess(setStyleForRank(new TranslatableComponent("commands.deathcounter.leaderboard.copy", "   " + finalRank + " - " + s + " (" + e.getKey() + ")"), finalRank), false); //setStyle
                        if(++done >= count) break;
                    }
                    if(done >= count) break;
        ...
        else
        {
            entities.stream().filter(e->e instanceof ServerPlayer).forEach(e -> {
                ...
                player.sendMessage(new TranslatableComponent("commands.deathcounter.leaderboard"), ChatType.SYSTEM, Util.NIL_UUID); //sendSystemMessage
                if(DeathCounter.deathHandler.getRankings().isEmpty())
                {
                    player.sendMessage(new TranslatableComponent("commands.deathcounter.leaderboard.none"), ChatType.SYSTEM, Util.NIL_UUID); //sendSystemMessage
                }
                else
                {
                    int done = 0;
                    ...
                    for(Map.Entry<Integer, TreeSet<String>> e1 : DeathCounter.deathHandler.getRankings().entrySet())
                    {
                        ...
                        for(String s : set)
                        {
                            ...
                            {
                                player.sendMessage(setStyleForRank(new TranslatableComponent("commands.deathcounter.leaderboard.copy", (s.equalsIgnoreCase(player.getName().getString()) ? "-> " : "   ") + rank + " - " + s + " (" + e1.getKey() + ")"), rank), ChatType.SYSTEM, Util.NIL_UUID); //sendMessage
                            }
                        }
